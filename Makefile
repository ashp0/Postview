...
IGNORE := $(shell mkdir -p $(BUILD); \
	[ "$$(cat $(CONFIG_STAMP) 2>/dev/null)" = "$(CONFIG_KEY)" ] || \
...
dist: verify-all sign package
	@echo "  dist     $(APP).zip built from a fully verified tree"

package: bandtool $(BENCHMARK) $(PROFILE) $(SHOWDOWN)
	@rm -rf "$(DISTDIR)"
	@mkdir -p "$(DISTDIR)"
...
stress: | $(BUILD)
	...
	@PV_STRESS_HOME=$$(/usr/bin/mktemp -d /tmp/postview-stress-home.XXXXXX) || exit 1; \
	  ...
	  HOME="$$PV_STRESS_HOME" PVSTRESS_DEADLINE_SCALE=$(DEADLINE_SCALE) \
	    $(BUILD)/stress/pvstress $(PDF) $(STRESSSCALE) || status=$$?; \
